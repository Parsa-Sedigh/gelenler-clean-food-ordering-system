...
class CustomerApplicationServiceImpl implements CustomerApplicationService {
    ...
    public CreateCustomerResponse createCustomer(CreateCustomerCommand createCustomerCommand) {
        /* Note: We kept things simple here. We don't use outbox pattern in this service. But for a strong consistency between
         local DB ops and the data publishing op, you still need to use outbox pattern along with CQRS as in the saga pattern.
         Because here after persisting the data into DB, you can't be sure if the publish op will be successful and if it's
         not successful, you will have inconsistency between the customer tables in customer svc and the order svc.*/
