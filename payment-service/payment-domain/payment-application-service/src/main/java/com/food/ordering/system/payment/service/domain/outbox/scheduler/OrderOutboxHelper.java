...
public class OrderOutboxHelper {
    ...
    /* As in the order svc, this method is called from schedulers and we have the @Transactional also there. But just to be safe,
    in case this helper is called from somewhere else without @Transactional, we also add the @Transactional here.

    Note: Default propagation method for a tx is required. So if there is a tx opened and the method with @Transactional is called,
    that same tx will be used. Otherwise, a new tx will be opened.*/
    @Transactional
    public void updateOutboxMessage(OrderOutboxMessage orderOutboxMessage, OutboxStatus outboxStatus) {
        ...
        log.info("Order outbox message status is updated as: {}", outboxStatus.name());
    }
