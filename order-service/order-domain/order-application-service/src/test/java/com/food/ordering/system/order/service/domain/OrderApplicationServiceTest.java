...
/* We use @TestInstance() to creat a single instance of this test class. By default, for each test method, a new instance of
the class is created. In that case, to be able to use beforeAll() method, you need to use a static method and to initialize the
fields in this beforeAll() method, you need to use static for each field. But we want to use beforeAll without the static modifiers and
to do this, we use TestInstance.Lifecycle.PER_CLASS that will allow this.

Note: OrderApplicationService and OrderDataMapper are real spring beans(not mocked) so they will be injected, including all
dependent spring beans. But the other fields are mocked beans because when we use @Autowired, spring will check the bean definitions in
the OrderTestConfiguration class and see them as mocked beans(we're returning their mocked versions).*/
...
public class OrderApplicationServiceTest {
    ...
    public void init() {
        ...
        /* Since we have mocked the customerRepository, we need to mock findCustomer method. So we use Mockito when() method and
        if findCustomer method is called, we tell it to return the object that we have defined(customer).*/
