...
public class OrderDataMapper {
    ...
    public Order createOrderCommandToOrder(CreateOrderCommand createOrderCommand) {
        /* Here, we don't set the orderId, trackingId and orderStatus. Those are the fields that we will set according to
        business rules on the domain service. For now, we only mapped the input values that we get from the clients and created
         an Order object.
         Remember: Entities are mutable, that means we can update this Order object later with the correct orderId, trackingId
         and status. */
